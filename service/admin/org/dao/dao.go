...
func Insert(db *gorm.DB, entity *dbModel.GOrg) (string, error) {
...
func Update(db *gorm.DB, query interfaces.QueryInterface) error {
	err := db.Model(&dbModel.GOrg{}).Select("name", "path", "icon").Where("id=?", query.GetID()).Updates(map[string]interface{}{
...
func Delete(db *gorm.DB, query interfaces.QueryInterface) error {
	...
	return db.Where("id=any(?)", query.GetIDs()).Delete(&dbModel.GOrg{}).Error
}
