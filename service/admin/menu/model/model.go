...
func (self *GMenu) Insert() (string, error) {
	var err error
...
func (self *GMenu) List(query *Query) ([]*ListModel, int64, error) {
	if query == nil {
		query = new(Query)
	}
	db := pgsql.Open()
...
func (self *GMenu) GetCount(db *sqlx.DB, query *Query, whereSql ...string) (int64, error) {
	if query == nil {
		query = new(Query)
	}
...
// 更新,根据用户id和数据id进行更新
// 部分字段不允许更新，userID, id
func (self *GMenu) Update(query *UpdateByIDQuery) error {
	if query == nil {
...
// 删除，批量删除
func (self *GMenu) Delete(query *DeleteQuery) error {
	if query == nil {
...
func (self *GMenu) ToggleDisabled(query *DisabledQuery) error {
	if query == nil {
		return errors.New("无操作条件")
	}
	if strings.TrimSpace(query.ID) == "" {
