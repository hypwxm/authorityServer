...
func (self *GAdminRole) GetByID(query *GetQuery) (*GetModel, error) {
	db := pgsql.Open()
...
func (self *GAdminRole) List(query *Query) ([]*ListModel, int64, error) {
	if query == nil {
		query = new(Query)
	}
	db := pgsql.Open()
...
func (self *GAdminRole) GetCount(db *sqlx.DB, query *Query, whereSql ...string) (int64, error) {
	if query == nil {
		query = new(Query)
	}
...
// 更新,根据用户id和数据id进行更新
// 部分字段不允许更新，userID, id
func (self *GAdminRole) Update(query *UpdateByIDQuery) error {
	if query == nil {
...
// 删除，批量删除
func (self *GAdminRole) Delete(query *DeleteQuery) error {
	if query == nil {
...
// 启用禁用店铺
func (self *GAdminRole) ToggleDisabled(query *DisabledQuery) error {
	if query == nil {
		return errors.New("无操作条件")
	}
	if strings.TrimSpace(query.ID) == "" {
